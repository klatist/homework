...
pthread_mutex_t matrixSum_lock;
...
int main(int argc, char *argv[]) {
  ...
  pthread_mutex_init(&matrixSum_lock, NULL);
...
void *Worker(void *arg) {
  ...
  ValuePosition max = {max.row= first, max.col= 0, max.val= matrix[first][0]};
  for (i = first; i <= last; i++) //för varej rad
    ...
    pthread_mutex_lock(&matrixSum_lock);
    matrixSum += total;
    ...
    pthread_mutex_unlock(&matrixSum_lock);
    return NULL;
